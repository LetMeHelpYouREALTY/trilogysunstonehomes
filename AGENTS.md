...
- **Monorepo root vs `web/`:** If the Vercel **Root Directory** is the repository root (not `web`), framework auto-detection looks at the root [`package.json`](package.json). That file includes `next`, `react`, and `react-dom` (same versions as [`web/package.json`](web/package.json)) so Vercel does not fall back to the **Other** preset. The root [`vercel.json`](vercel.json) `installCommand` runs `npm install` at the repo root, then `cd web && npm install` for the app. Prefer **Root Directory = `web`** when possible so only [`web/vercel.json`](web/vercel.json) and `web/package.json` matter.
